...
int main()
{
  ...
  while(i++<2){ //change this with a reasonable condition !!!
 ...
 if(ret>0)
 {
   while(1){ //this while(1) is ok - it has been used with a clear purpose
    ...
    //first parameter can also be -1 - in this case, any terminated
    //child process of this parent process will  be cleaned-up 
    ...
    if(WIFEXITED(status))     //normal termination of the process
    {
       if(WEXITSTATUS(status) == 0){ //normal and successfull
          printf("child terminated normally and successfully and pid is %d\n",ret);
       }
       else{//normal, but not successfull
           //you may have to set a flag to be used later 
          printf("child terminated normally, but not successfully and pid is %d\n",ret);
       }
    }
    else{ //abnormal (did not succeed)
        //you may have to set a flag here to be used later
       printf("child terminated abnormally and pid is %d\n",ret);
    }
   }//ret>0
    //we may use break instead of exit(0) 
    if(ret<0) { exit(0); } //no child is in any state for this process
