...
//Note: after understanding and testing this example,
//      complete the same by replacing condition variables
//      with 2 semaphores - observe what is the difference !!!
...
pthread_cond_t eq_cv1,dq_cv2;  //enqueue and dequeue condition 
                               //variables !!!
	...
	while(1)
	{
		struct job* next_job;
        	...
        	pthread_mutex_lock(&m1);
                ...
                //pthread_cond_wait() always blocks the 
                //current thread unconditionally, when invoked !! 
     ...
     printf("the string is %s\n", next_job->str1);
                ...
                //if there is no target thread blocked on 
                //this condition variable, the notification  will be 
                //lost 
...
int main()
{
	...
	pthread_attr_init(&tha1); // initialize thread attribute object - default
	pthread_mutexattr_init(&ma1);//initialize mutex attribute to default attributes !!
	...
	//pthread_mutex_init(&m1,NULL);
        ...
        //returns !!!
        //pthread_mutex_lock() will block the current thread(in wq of
        //this mutex), if the lock state is locked - 
        //if another thread uses pthread_mutex_unlock(),
        ...
        //aspect is that there is no increment operation  or decrement operation 
	...
	pthread_mutex_init(&m1,&ma1);
        ...
        //a condition variable is an abstract object, which 
        //does not contain a counter - which does not contain
        //a lock state - it is just an object - however, it is 
        //effective with proper coding and a mutex - the usage 
        //will be clear, in the code 
        //for the abstract condition variable, there will 
        //a wq in the system space - the usage of the wq 
        //will be clear in the code !!!
        //the associated mutex will take care of any locking
        //along with the usage of condition variable - meaning,
        //condition variable cannot be used as a lock and 
        //it also cannot be used as a counter !!!
        //condition variable is just used for synchronization,
        //as per custom conditions provided by the developer !!
        //best way to understand a condition variable is 
        //with the help of operations !!!        
