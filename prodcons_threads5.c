...
void *user_input_thread(void *arg)
{
   ...
   while(1){
     ...
     ret = (int) fgets(pdatabuf,sizeof(pdatabuf),stdin);    //produce
...
void *display_status_thread(void *arg)
{
   ...
   while(1){
        ...
        sleep(3); //execute every 3 seconds 
   }
...
int main()
{
   ...
   int ret;
   struct sched_param p1,p2,p3,p4,p5;
   ...
   ret = pthread_setschedparam(pthread_self(),SCHED_FIFO,&p1);
   if(ret != 0) { printf("error in scheduling parameter setting\n"); exit(2); }
   ...
   //library !!! refer to man pages for default settings
   //of attributes !!!
 ...
 if(ret>0) { printf("error in thread creation for producer\n"); exit(3); } 
   ...
   //pthread_join() may used on any joinable of this process
