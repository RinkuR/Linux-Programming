...
void *display_status_thread(void *arg)
{
   ...
   //every thread inherits signal mask of the main() thread(creating 
   //thread - in our case, main() thread is the creating thread !!

   //using pthread_sigmask(), we can customize private signal 
   //mask of the respective thread !!!  
...
int main()
{
   ...
   //all threads created after this setup will inherit the 
   //signal mask field of the creating thread - in this case,
   //main() thread is the creating thread !!!

   //install all the signal handlers here- before any thread is 
   //created ????
   

   //param1 is pointer to user-space thread id (it may be an
   //object maintained by user-space library) 
    

   //param2 is used to set the attributes - known as thread 
   //attributes object  - we will see later
   //if param2 is NULL, default attributes are used by the 
   //library !!! NON NULL for custom attributes - 
   //refer to man pages for default settings
   //of attributes !!!
   ...
   //not a stack area !!! this parameter is useful for 
   //reentrancy !!!
   ...
   //what about the user-space thread stack for this newly 
   //created thread ???
   //by default, thread library will assign a default 
   //stack size and pass it to the underlying system call API !!!
   //however, you may wish to customize the stack size, if needed !!!
 ...
 if(ret>0) { printf("error in thread creation for producer\n"); exit(3); } 
   ...
   //pthread_join() may also clean-up resources allocated 
   //by lib, implicitly !!!
   ...
   //you may need to do additional clean-up on behalf of 
   //threads of a given process, here !!
