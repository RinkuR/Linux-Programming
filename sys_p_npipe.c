...
int main(int argc, char *argv[])
{
   ...
   //has an inode and directory entry - the type will be
   //encoded in the inode, which is used by the logical fm
   //and regular file manager !!!
   ...
   //there may not be a file system manager in this context !!! 


   ret = mkfifo(argv[1],0600);
   //ret is just the return value used for error checking 
 
  //after a open() system call API on a name pipe file, 
  ...
  //in doing so, after the  setup is created, 
  //current process is blocked in the wq of the 
  //pipe object until another process opens the pipe file
  //for read or write , accordingly - this is a rule for 
  //named pipe IPC, which is natural - meaning, 2 or more
  //processes must be involved in reading/writing !!! this
  //is not a regular file, where one process can just
  //deal with the file !!! 
  ...
  //you can always setup multiple pipe IPC mechanism instances to 
  ...
  //this is an implicit synchronization !!!

  //if there is no data in the pipe buffer and there is a write process
  //with a write handle to keep this pipe alive, end of file will never
  //be seen by the read process !!! meaning, read() process is blocked
  //in wq of pipe object !!

  //if the write process closes the write handle to this pipe or
   //the write process terminates, the read process will see an
  //end of file(ret will be returning 0),when the pipe buffer is empty  - 
  //this is a subtle difference with respect to 
  //regular files 

  //unlike in the case of regular files, random access of pipe
  //buffer is not allowed - meaning, lseek() or similar system call
  //APIs will return error !!! meaning, byte offset field of 
  //open file object is unused !!

  //the following comments apply to a process that writes to 
  //the pipe :

  //if there is no space in the pipe buffer and a process attempts
  //to write to the pipe, current process will be blocked - in this
  //case we assume that another process is holding a read handle
  //to this named pipe !!
