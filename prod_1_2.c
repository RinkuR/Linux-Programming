...
int main()
{
  ...
  //in this example/assignment 3 semaphores are used in a semaphore object 
  ...
  // speciality of  a counting semaphore is that it can keep 
  // count of a set of identical resources and also block the 
  // current process, if the available resources is 0(none)
  //
  //in addition, a counting semaphore also can do one more 
  //action - that is, it can wake up a blocked process, if 
  ...
  //initialize and operate upon the counting semaphores
  //appropriately !!!  in our case, we are using the first 
  //semaphore to keep count of free slots and to do, we initialize
  ...
  //replace the process semaphore APIs with thread semaphore APIs
  //also use mutex related APIs to initialize the mutex !!!
