...
voiid sig_chld(int signo)
{
  ...
  while(1){
      ...
      //waitpid() will be transformed into a non-blocking system call
      //API, if there are children and
      ...
      //ret1 = waitpid(-1,&status,0); //wrong approach !!!
      ret1 = waitpid(-1,&status,WNOHANG); //correct approach !!!
      //ret1 = waitpid(-1,&status,0);  //wrong approach 
      if(ret1<0) break;
      if(ret1==0) break;
      if(ret1>0) count++;  //this count may be used somewhere in 
                           //the main() to take further actions !!!
  }
...
int main()
{
   ...
   npsfd = open(argv[1], O_RDONLY);  //may block here !!! 
   if(npsfd<0) { 
   ...
   while(1)
   {
      ...
      //by &msgbuf1 !!!
       ...
       //fork and let the child handle the client's request - 
