...
int main()
{
   ...
   if(ret<0){ perror("error in fork"); exit(3); }
   ...
   //while(1); 
           ...
           //the first parameter can also be +ve(specific pid of 
           //a child process)- in this case, 
...
//           pause(); //used to block a process, unconditionally  !!!
   ...
   //ret = execl("/usr/lib/ooo3/program/soffice.bin","soffice.bin",NULL);  
   //if(ret<0) { perror("error in execl");  exit(33); }     
   ret = execl("/usr/bin/dolphin","dolphin","/home/babu/",NULL);  
   //ret = execl("/usr/bin/gcc","gcc","fork11.c", "-o", "fork11",NULL);  
   if(ret<0) { perror("error in execl"); exit(8); }     
