...
int main()
{
 ...
 //as per our conventions, we do not initialize 
 //shared memory fields and semaphores, in consumer - initialize only in
 //producer 
  ...
  printf("the actual values of the semaphores are %d, %d, %d\n", \
    ...
    //in this coding, locking works perfectly and our critical 
    //section problem is solved, but a new problem 
    //is created !!
    //meaning, this process will lock , check for valid data slots
    //and read, if available - if not , continue using lock and 
    //checking the conditions - continue this again and again !!! 
    //
    //this will lead to busy waiting and wastage of cpu cycles
    //such inefficient busy waiting is not acceptable in 
    //GPOS and RTOS - in both cases, we need to code more 
    //efficiently !!!

    //if the below condition is false, current process must 
    //be blocked and also woken up when the condition is true !!!
    //however, it is not being done so, in this context ???
    //
    //we need to fix this problem, using more semaphores -
    //refer to prod_1_2.c and cons_1_2.c for such a solution !!!
